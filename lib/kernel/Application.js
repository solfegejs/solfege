...
proto.parseSolfegeUri = function(uri, bundleCaller)
{
    ...
    if (filePath) {
        ...
        // Try glob search and return multiple files
        try {
            var globPaths = glob.sync(filePath, {
                cwd: basePath
            });
            var absoluteFilePaths = [];
            if (globPaths && globPaths.length) {
                globPaths.forEach(function(globPath) {
                    absoluteFilePaths.push(nodePath.resolve(basePath, globPath));
                });
            }
            if (absoluteFilePaths.length === 1) {
                return absoluteFilePaths[0];
