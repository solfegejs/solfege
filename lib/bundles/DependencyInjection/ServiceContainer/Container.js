...
class Container {
    ...
    *buildInstance(definition) {
        ...
        var classArguments = definition.getArguments();

        // Resolve arguments
        var classArgumentsResolved = [];

        if (!(classArguments && (typeof classArguments[Symbol.iterator] === 'function' || Array.isArray(classArguments)))) {
            throw new TypeError("Expected classArguments to be iterable, got " + _inspect(classArguments));
        }
        ...
        try {
            for (var _iterator4 = classArguments[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                var classArgument = _step4.value;

                var classArgumentResolved = yield this.resolveParameter(classArgument);
        ...
        try {
            ...
            instance = _harmonyReflect2.default.construct(classObject, classArgumentsResolved);
        } catch (error) {
    ...
    *resolveParameter(parameter) {
        ...
        var resolvedParameter = void 0;
        var singlePropertyMatched = parameter.match(/^%([^%]+)%$/);

        function _ref3(match, propertyName) {
            var propertyValue = _this.resolveConfigurationProperty(propertyName);
        ...
        if (Array.isArray(singlePropertyMatched)) {
            ...
            resolvedParameter = parameter.replace(/%([^%]+)%/g, _ref3);
        }

        // Check number
        var numberCast = Number(resolvedParameter);
     ...
     */
    resolveConfigurationProperty(propertyName) {
        ...
        var property = this.configuration;
        ...
        try {
            for (var _iterator7 = propertySplittedName[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
